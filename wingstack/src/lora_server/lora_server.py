...
# store the erroneus packets in a buffer
ERROR_BUFFER = []
...
def handle_data_up(gw_id, data_frame, rxpk_obj):
    ...
    # update framecount mark
    session.fcnt_mark += 1

    # Batched ack
    # Determine if the MCMD_BATCH_ACK_REQ command is present in fOpts
    if len(data_frame.fOpts) > 0 and data_frame.fOpts[0] == 0x14:
        session.batchAckRequested = True
        print('Feedback ACK requested')
    else:
        session.batchAckRequested = False
        print('No ACK')
    ...
    # logic for feedback
    cur_fcnt = int.from_bytes(data_frame.fCnt, "little")

    # check frame loss
    if session.fcnt_mark != cur_fcnt:
        session.feedback_buffer.append(0)
        session.fcnt_mark = cur_fcnt
        
    if mic_calc == data_frame.mic:
        session.feedback_buffer.append(2)
        print('MIC validation passed')

        # Update counter if batched ACK is requested
        if session.batchAckRequested:
            session.ackCounter += 1

        # send ack where batchAckRequested not present
        # or only for every other frame where batchAckRequested
        if session.batchAckRequested:
            print('Sending feedback for previous 8 frames')
            feedback = 0
            for status in session.feedback_buffer:
                feedback = (feedback << 2) | status
            # here we further encode the feedback to 2 bytes
            encoded_feedback = feedback.to_bytes(2, 'big')
            # customized mac command
            mac_feedback = b'\x15' + encoded_feedback

            fcntdl_full = session.fCntDown.to_bytes(4, 'little')
            ack = DataFrame(b'\x60', data_frame.devAddr, b'\x23', fcntdl_full[0:2], mac_feedback, b'\x00', b'')
    ...
    # mic check failed
        ...
        session.feedback_buffer.append(1)
...
def _majority_voting(byte_array_list):
    if len(byte_array_list) < 3:
        raise ValueError("At least three buffer must be provided")

    buffer_size = len(byte_array_list[0])

    for buffer in byte_array_list[1:]:
    ...
    for i in range(buffer_size):
        for bit in range(8):
            ...
            count_ones = 0
            for buffer in byte_array_list:
                if buffer[i] & mask != 0:
                    count_ones += 1

            if count_ones >= len(byte_array_list) // 2 + 1:
                result[i] |= mask

    return result


def correct_data(rxpk, EXP_T = 10):
    if time.time() - ERROR_BUFFER[-1][1] > EXP_T:
        ERROR_BUFFER.clear()

    ERROR_BUFFER.append(bytearray(base64.b64decode(rxpk['data']), time.time()))

    corrected_res = b""
    if len(ERROR_BUFFER) >= 3:
        try:
            print('Correcting data...')
            corrected_res = _majority_voting(ERROR_BUFFER)
        except ValueError as ve:
            print(ve)
    
    return base64.b64encode(corrected_res).decode('utf-8')
    ...
    if 'rxpk' in push_obj:
        for rxpk in push_obj['rxpk']:
            ...
            if 'crc_match' in rxpk and rxpk['crc_match'] != 1:
                corrected_data = correct_data(rxpk)
                if corrected_data:
                    rxpk['data'] = corrected_data
                else:
                    continue
